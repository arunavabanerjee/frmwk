    	...
    	<table border="0" width="100%" cellspacing="0" cellpadding="0">
			...
			<?php define(MAX_DISPLAY_SEARCH_RESULTS_LOCAL, 200);
  			if (isset($HTTP_GET_VARS['page']) && ($HTTP_GET_VARS['page'] > 1)) 
    			...
    			$products_query_raw = 
    				"select p.products_id, p.products_model, p.products_quantity, p.products_price, pd.products_name, count(pa.products_attributes_id) as attributes ".
				...
				$rows = 0; $products_query = tep_db_query($products_query_raw);
          		...
          		<tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href='<?php echo tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $products['products_id'] . '&origin=' . FILENAME_POS_PRODUCTS_WHOLESALE_PRICE . '?page=' . $HTTP_GET_VARS['page'], 'NONSSL'); ?>'">
                	...
                	<td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $products['products_id'] . '&origin=' . FILENAME_POS_PRODUCTS_WHOLESALE_PRICE . '?page=' . $HTTP_GET_VARS['page'], 'NONSSL') . '">' . $products['products_name'] . '</a>'; ?></td>
                	<td class="dataTableContent"><?php echo $products['products_model']; ?></td>

					<td class="dataTableContent"><?php echo $products['attributes']; ?></td>
                	...
                	<td class="dataTableContent"><?php echo $products['products_quantity'] * $products['products_price']; ?></td>
          		</tr>
			<?php } ?>
    		</table>
    		</td></tr>
