...
//                $redirect->headers->clearCookie(CartService::CART_COOKIE_NAME);
	...
	$cartService->setCookie($request->cookies->get(CartService::CART_COOKIE_NAME, ''));
        ...
        //\Symfony\Component\VarDumper\VarDumper::dump($request->query); exit; 

        //-------------------
        // afterpay checkout 
        //-------------------
        if( ($request->query->has('status')) && ($request->query->has('orderToken')) ){ 
            if( ($request->query->get('status') == 'SUCCESS') && ($request->query->get('orderToken') != NULL )){  
                ...
                try{
                    $apiresponse = $afterpayClient->request(Request::METHOD_POST, '/v1/payments/capture', [ 
                        'headers' => [
                   ...
                   // response for /v1/payments/capture  -- success order
                   $afterpayResponse = json_decode((string) $apiresponse->getBody(), true); //dump($afterpayResponse); exit

                }catch(\GuzzleHttp\Exception\RequestException $re){ //dump($re); 
                    $expbody = json_decode((string) $re->getResponse()->getBody(), true); //dump($expbody);
                    //if error code is declined, then redirect to cart page. 
                    if($expbody['errorCode'] == 'declined'){ 
                        $this->addFlash('error','Payment Not Approved. Afterpay has been unsuccessful, please use another method to checkout.');    
                        $redirect = $this->redirectToRoute('cart_page');
                        $redirect->headers->setCookie($cartService->getCartCookie());
                        $redirect->headers->setCookie($cartService->getCountryCookie());
                        $redirect->headers->setCookie($userService->getUserCookie());
                        ...
                        $this->addFlash('error','Payment Not Approved. Afterpay has been unsuccessful, please use another checkout method.');    
                        $redirect = $this->redirectToRoute('cart_page');
                        $redirect->headers->setCookie($cartService->getCartCookie());
                        $redirect->headers->setCookie($cartService->getCountryCookie());
                        $redirect->headers->setCookie($userService->getUserCookie());
                ...
                if( $afterpayResponse['status'] == 'APPROVED' ){ //dump($afterpayResponse); exit; 
                ...
                $this->addFlash('error','Afterpay has been unsuccessful. Please Use Another Method To Checkout.');
                $redirect = $this->redirectToRoute('cart_page');
                $redirect->headers->setCookie($cartService->getCartCookie());
                $redirect->headers->setCookie($cartService->getCountryCookie());
                $redirect->headers->setCookie($userService->getUserCookie());
        ...
        } // end afterpay checkout


        //------------------
        // zippay checkout
        //------------------  
        if( ($request->query->has('result')) && ($request->query->has('checkoutId')) ){ 
            // Configure API key authorization: Authorization
            //\Symfony\Component\VarDumper\VarDumper::dump($request->query);
            ...
            try { 
                ...
                if($apiResult == 'approved'){ 
                    ...
                    try { 
                        ...
                        $oretailRepository = $this->container->get('App\Repository\OrderRetailRepository');
                        $norder = $oretailRepository->find($checkoutOrderId); $norder->setOrdersStatus(1);
                        $entityManager = $this->container->get('doctrine.orm.default_entity_manager');
                        $entityManager->flush();
...
//        }
        ...
        //\Symfony\Component\VarDumper\VarDumper::dump($this->get('session')); exit;
