...
if(isset($_SESSION['items_in_sale'])) {
  ...
  foreach($item_info as $item){ if(empty($item)){ continue; }
    ...
    $retailtax = number_format(floatval(($itemDet[4] * $itemDet[2] * $itemDet[3])/100), 2, '.', '');  
    ...
    $carthtml .= '</tr>'; 
    $sub_total += $itemDet[2] * $itemDet[4]; $tax_total += $retailtax; $grand_total += $itemDet[5];
  }
  ...
  $discounted_total = number_format(floatval($_SESSION['disc_grandtotal']), 2, '.', '');
}
...
jpost(document).on("click", '.search__table .deletepro', function(event){ 
   ...
   jpost.post("sale_ui.php", { pdelid : value, del : true })
        .done(function(data){ console.log(data); });
   //calc totals
   var curr_subtotalretail = 0.00; var curr_taxtotalretail = 0.00; 
   var curr_grandtotalretail = 0.00;
   jpost('.search__table tbody tr').each(function(){
     if(jpost(this).find('td.uprice').text() != 0.00){
        curr_subtotalretail += (parseFloat(jpost(this).find('td.uprice').text()) * parseInt(jpost(this).find('input[type=number]').val()));
     } /*else{
        curr_subtotalretail += parseFloat(jpost(this).find('td.totalprice').text());	
     }*/ 
   }); 
   curr_taxtotalretail = ((parseInt(taxpc) * curr_subtotalretail)/100); 
   jpost('.search__table tbody tr').each(function(){
     if(jpost(this).find('td.uprice').text() != 0.00){
        curr_grandtotalretail += parseFloat(jpost(this).find('td.totalprice').text());
     } else{
        curr_grandtotalretail += parseFloat(jpost(this).find('td.totalprice').text());	
     } 
   }); 
   //update totals 
   jpost('.sels__stab__comnt_right .sub-total').text(curr_subtotalretail.toFixed(2));
   jpost('.sels__stab__comnt_right .sales-tax').text(curr_taxtotalretail.toFixed(2));
   jpost('.sels__stab__comnt_right .grand-total').text(curr_grandtotalretail.toFixed(2));
   jpost('#amount-tendered').val(curr_grandtotalretail.toFixed(2));
   //apply discount is applicable
   var pcdiscount = parseInt(jpost('#promocodeval').val());
   if(pcdiscount != ''){ if(pcdiscount > 0){ jpost('#applycc').trigger('click'); }}
   //updating the session values
   jpost.post("sale_ui.php", {updtses:true,grandtotal:curr_grandtotalretail,subtotal:curr_subtotalretail,taxtotal:curr_taxtotalretail})
        .done(function(data){ console.log(data); });
});
jpost(document).on("keyup", '.search__table .quantity', function(){ 
  ...
  if((parseFloat(unitprice) > 0.00) && (parseInt(enteredqty) > 0)){ 
    ...
    //calc totals
    var curr_subtotalretail = 0.00; var curr_taxtotalretail = 0.00; var curr_grandtotalretail = 0.00;
    jpost('.search__table tbody tr').each(function(){
      if(jpost(this).find('td.uprice').text() != 0.00){
         curr_subtotalretail += (parseFloat(jpost(this).find('td.uprice').text()) * parseInt(jpost(this).find('input[type=number]').val()));
      } /*else{
