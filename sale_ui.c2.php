...
if(!empty($_POST['item_search_pname']) || !empty($_POST['item_search_pmpn']) 
	...
	if(!empty($bcode)){ 
		$from .= ',products_description pd,products_attributes pa'; 
		$search .=" and p.products_id = pa.products_id and pa.barcode like \"%$bcode%\" "; 
	}
	if(!empty($man)){ 
	...
	$query="SELECT p.products_id,pd.products_name,p.products_price,tr.tax_rate
		FROM products p, tax_rates tr, tax_class tc"; 
        $query .= $from;
	...
	//sale price added 
	$query="SELECT p.products_id,pd.products_name,p.products_price,tr.tax_rate
		FROM products p,tax_rates as tr,tax_class as tc";
	$query .= $from;
	/*$query .= " WHERE p.products_status = '1' and p.products_quantity > 0 and p.products_id=pd.products_id and
			p.products_tax_class_id=tc.tax_class_id and tr.tax_class_id=tc.tax_class_id "; */
	$query .= " WHERE p.products_quantity > 0 and p.products_id=pd.products_id and
	...
	$query="SELECT p.products_id,pd.products_name,p.products_price,tr.tax_rate
		FROM products_description as pd,products p,tax_rates as tr,tax_class as tc
...
while($row=mysql_fetch_assoc($item_result)){
	...
	$item_data .= "<li value='$option_value'><a href='javascript:void(0)'>$display_item</a></li>";
}
$item_data .= "</ul>";*/

$item_result=mysql_query($query,$dbf_osc->conn); //$count_items = mysql_num_rows($item_result);
...
while($row=mysql_fetch_assoc($item_result)){
	$id=$row['products_id']; 
        $query = "SELECT pa.products_id,pa.products_attributes_id,pa.options_id,pa.options_values_id, 
		  ...
		  WHERE products_id='".$id."' AND po.products_options_name like '%-Shop' AND pa.quantity > 0
		  ORDER BY po.products_options_name ASC";
	$attr_result=mysql_query($query,$dbf_osc->conn); $count_aitems = mysql_num_rows($attr_result);

  	//sale price added
	//if ($row['specials_new_products_price']){ $unit_price=$row['specials_new_products_price']; } 
	//else{ $unit_price=$row['products_price']; }
	$unit_price=$row['products_price']; $tax_percent=$row['tax_rate'];
  	$option_value="$id"."/"."$unit_price"."/"."$tax_percent"; //$option_value="$id"; 
	$display_item="$row[products_name]"; 
	//total number of items
        //$total_items = 0; 
	//while($attrrow=mysql_fetch_assoc($attr_result)){ $total_items += $attrrow['quantity']; }
	...
	if(!empty($count_aitems)){
	  if($count_aitems == 1){ //if count aitems == 1 
		...
		if(strstr($attrrow['products_options_name'], "NoAttributes-Shop")){
		   $option_value .= '/'.'pid:'.$attrrow['products_id'].'-'.$attrrow['products_attributes_id'].
						'-'.$attrrow['options_id'].'-'.$attrrow['options_values_id'].','; 
		   $option_value .= 'optprice:'.$attrrow['options_values_price'].',';
 		   $option_value .= 'quantity:'.$attrrow['quantity'].',';
		   $option_value .= 'optname:'.$attrrow['products_options_name'].',';	
		   $option_value .= 'optvalname:'.$attrrow['products_options_values_name'];
	
		   $item_data_1 .= "<li value='$option_value'><a href='javascript:void(0)'>$display_item</a></li>";
		   ...
		   $optname = strstr($attrrow['products_options_name'], ' ') ? 
				str_replace(' ','\040',$attrrow['products_options_name']) : $attrrow['products_options_name'];
		   $attr_option_value = $option_value.'/'.'pid:'.$attrrow['products_id'].'-'.$attrrow['products_attributes_id'].
						'-'.$attrrow['options_id'].'-'.$attrrow['options_values_id'].','; 
		   $attr_option_value .= 'optprice:'.$attrrow['options_values_price'].',';
 		   $attr_option_value .= 'quantity:'.$attrrow['quantity'].',';
		   $attr_option_value .= 'optname:'.$optname.',';	
		   $attr_option_value .= 'optvalname:'.$attrrow['products_options_values_name'];
...
if(isset($_POST['pid'])){ //echo $_POST['pid']; 
  ...
  if(!isset($_SESSION['current_sale_customer_id']) && empty($sale_items)){ 
    ...
    if($_POST['pid'] == 0){ 
	   ...
	   $pid = strval(1000001); $notFoundItems = 1; 
	   $_SESSION['not_found_items'] = $notFoundItems;	
        } else{ 
	   $count = $_SESSION['not_found_items']; $pid = strval(1000001 + $count); 
	   $_SESSION['not_found_items'] = ++$count;  
	} 
	...
	$pQuery = "SELECT p.products_price, pd.products_name FROM products as p,products_description as pd 
        ...
        $pquant = 1;
	$sale_items .= $_POST['pid'].','.$pResult[1].','.$uprice.','.$retailtaxpc.','.$pquant.','.$retailprice.';;';
        $_SESSION['items_in_sale'] = $sale_items; 
	//append values
    	$html = '<tr id="'.$_POST['pid'].'">';
    	$html .= '<td class="pronamedtls"><span class="d-block">'.$pResult[1].'</span></td>';
    	$html .= '<td class="uprice">'.$uprice.'</td><td class="taxpc">'.$retailtaxpc.'</td>';
...
if(isset($_POST['del'])){ 
 if($_POST['del']){ 
  ...
  $_SESSION['items_in_sale'] = $sale_items;
  $html = 'Item with ID: '.$_POST['pdelid'].' removed';
  echo $html; die();
 }
}
...
if(isset($_POST['updtses'])){
 if($_POST['updtses']){
   ...
   if(isset($_POST['updtsession']) && !empty($_POST['updtsession'])){ 
      if(isset($_SESSION['items_in_sale']) && !empty($_SESSION['items_in_sale'])){ 
        $sale_items = $_SESSION['items_in_sale']; $sale_items .= $_POST['updtsession'];
        $_SESSION['items_in_sale'] = $sale_items;
      } else { //if session empty
	$_SESSION['items_in_sale'] = $_POST['updtsession'];
      }
   }
   ...
   if(isset($_POST['updaterow']) && !empty($_POST['updaterow'])){
     $sale_items = $_SESSION['items_in_sale']; $sale_items_arr = explode(';;',$sale_items); 
     $sale_items_narr = array();
     foreach($sale_items_arr as $sale_item){ 
       if(empty($sale_item)){ continue; }
       if(strstr($sale_item,$_POST['updaterow'])){ 
	 $saleItemArr = explode(',',$sale_item); 
         $saleItemArr[4] = $_POST['quantity']; $saleItemArr[5] = $_POST['rowtotal']; 
         $sale_item_n = implode(',',$saleItemArr);
         array_push($sale_items_narr,$sale_item_n);
       }else{
         array_push($sale_items_narr,$sale_item); 
       }
     }
     $sale_items = implode(';;',$sale_items_narr); $sale_items = $sale_items.';;';
     $_SESSION['items_in_sale'] = $sale_items; $html = 'Item with ID: '.$_POST['updaterow'].' updated';
   } 
   echo $html; die(); 
 }
}
...
if(isset($_SESSION['items_in_sale'])) {
  ...
  $discount = number_format(intval($_SESSION['pcdiscount']), 2, '.', '');
  $discounted_total = number_format(intval($_SESSION['disc_grandtotal']), 2, '.', '');
}
...
<div class="container-fluid">	
	...
	<div class="searchtable">
		...
		<?php if( empty($_SESSION['items_in_sale']) || $_SESSION['items_in_sale'] == ';;' ){ 
	...
	<div class="col-5 sels__search_tablebottom_right">
		...
		<tr id="discount_field">
		    ...
		    <td class="text-left text-sucess themecolor font-weight-bold">
			<span class="lnr lnr-plus-circle" id="showcoupon">
			 <?php if(isset($_SESSION['pcdiscount']) && !empty($_SESSION['pcdiscount'])){ echo $discount; } ?>
			</span>
		    </td>
		</tr>
		...
		<tr>
			...
			<?php if(isset($_SESSION['disc_grandtotal']) && !empty($_SESSION['disc_grandtotal'])){ 
...
jpost(document).ready(function(){ 
  //apply discount is applicable
  var pcdiscountOnLoad = parseInt(<?php if(isset($_SESSION['pcdiscount'])){ echo $_SESSION['pcdiscount']; } ?>); 
  //alert(pcdiscountOnLoad);
  if(pcdiscountOnLoad > 0){ 
    jpost('#coponcode_field #promocodeval').val(pcdiscountOnLoad); 
    jpost('#coponcode_field #applycc').trigger('click'); 
  }

  //stylize option lists
...
jpost('.searpro__result__list__dtls li').click(function(){ 
  ...
  if(jpost(this).parent().parent().hasClass('collapse')){ 
     name = jpost(this).parent().parent().parent().find('a').text();
  }
  ...
  var licarthtml = createCartRow(value1, name);
  jpost('.search__table tbody').append(licarthtml); 
  //make calculations and add to grand total 
  var uprice = jpost(licarthtml).find('td.uprice').text(); //console.log(uprice); 
  var taxpc = jpost(licarthtml).find('td.taxpc').text(); //console.log(taxpc);
  var quant = jpost(licarthtml).find('input[type="number"]').val(); //console.log(quant);
  var price = jpost(licarthtml).find('td.totalprice').text(); //console.log(price);
  //cals tax
  ...
  jpost('#amount-tendered').val(grandtotalretailn.toFixed(2)); 
   //apply discount is applicable
   var pcdiscount = parseInt(jpost('#promocodeval').val());
   if(pcdiscount != ''){ if(pcdiscount > 0){ jpost('#applycc').trigger('click'); }}
  ...
  var end = new Date().getTime(); console.log(end - start);
});
function createCartRow(data, name){
  var $html=''; var dataArray = data.split('/');
  var uprice = parseFloat(dataArray[1]); 
  var retailtaxpc = parseInt(dataArray[2]);
  var retailprice = uprice + ((retailtaxpc * uprice)/100);
  var optionArray = dataArray[3].split(','); 
  var pid = optionArray[0].replace('pid:',''); 
  var optName = optionArray[3].replace('optname:','');	
  var optValName = optionArray[4].replace('optvalname:',''); 
  //if(optValName.indexOf('\040') > 0 ){ optValName = optValName.replace('\040', ' '); }
  //append values
  $html = '<tr id="'+pid+'">';
  $html += '<td class="pronamedtls"><span class="d-block">'+name+'</span>';
  $html += '<span class="d-block">'+optName+' : '+optValName+'</span></td>';
  $html += '<td class="uprice">'+uprice.toFixed(2)+'</td><td class="taxpc">'+retailtaxpc+'</td>';
  $html += '<td><div class="quantity"><input type="number" min="1" max="9" step="1" value="1"></div></td>';
  $html += '<td class="totalprice">'+retailprice.toFixed(2)+'</td>';
  $html += '<td class="deletepro"><i class="lnr lnr-cross-circle"></i></td>';
  $html += '</tr>'; //console.log($html); 
  //updating the session values
  sale_items = ';;'+pid+','+name+','+uprice+','+retailtaxpc+','+'1'+','+retailprice+';;';	
  jpost.post("sale_ui.php", { updtses : true, updtsession : sale_items })
       .done(function(data){ console.log(data); });
  return $html;
}
jpost(document).on("click", '.search__table .deletepro', function(event){ 
   ...
   var notice = '<p style="color:red;font-weight:bold;"> Item with ID: '+value+' removed </p>';
   jpost('.pos__search__result .messages').append(notice);
   setTimeout(function(){ jpost(".pos__search__result .messages").fadeOut().empty(); }, 500);
   //apply discount is applicable
   var pcdiscount = parseInt(jpost('#promocodeval').val());
   if(pcdiscount != ''){ if(pcdiscount > 0){ jpost('#applycc').trigger('click'); }}
   ...
   jpost.post("sale_ui.php", { pdelid : value, del : true })
        .done(function(data){ console.log(data); });
   //updt session
   jpost.post("sale_ui.php", { updtses : true, grandtotal : grandtotalretailn, subtotal : subtotalretailn, taxtotal: taxretailn })
	.done(function(data){ console.log(data); }); 
   var end = new Date().getTime(); console.log(end - start);
});
var prevqty = -1;
jpost(document).on("keydown",'.search__table .quantity', function(){ 
  prevqty = jpost(this).find('input[type=number]').val(); console.log(prevqty);
});
jpost(document).on("keyup", '.search__table .quantity', function(){ //console.log(prevqty);
  var pid = jpost(this).parent().parent().attr('id'); //console.log(pid);
  var enteredqty = jpost(this).find('input[type=number]').val(); //console.log(enteredqty);
  var unitprice = jpost(this).parent().parent().find('.uprice').text(); //console.log(unitprice);
  var taxpc = jpost(this).parent().parent().find('.taxpc').text(); //console.log(taxpc); 
  var prevtotalprice = jpost(this).parent().parent().find('.totalprice').text(); //console.log(prevtotalprice);
  //get existing totals
  var prevprodtax = (parseInt(taxpc) * parseFloat(unitprice) * parseInt(prevqty))/100; //console.log(prevprodtax);
  var prevsubtotal = jpost('.sels__stab__comnt_right .sub-total').text(); //console.log(prevsubtotal);
  var prevtaxtotal = jpost('.sels__stab__comnt_right .sales-tax').text(); //console.log(prevtaxtotal);
  var prevgrandtotal = jpost('.sels__stab__comnt_right .grand-total').text(); //console.log(prevgrandtotal);
  //current totals
  var currprodtax = (parseInt(taxpc) * parseFloat(unitprice) * parseInt(enteredqty))/100; //console.log(currprodtax); 
  var currsubtotal = parseFloat(prevsubtotal) - (parseFloat(unitprice) * parseInt(prevqty)) +  (parseFloat(unitprice) * parseInt(enteredqty)); 
   //console.log(currsubtotal);
  var currtaxtotal = parseFloat(prevtaxtotal) - parseFloat(prevprodtax) + parseFloat(currprodtax); //console.log(currtaxtotal);
  var currrowtotal = (parseFloat(unitprice) * parseInt(enteredqty)) +  
                         ((parseInt(taxpc) * parseFloat(unitprice) * parseInt(enteredqty))/100) ; //console.log(currrowtotal);
  var currgrandtotal = parseFloat(prevgrandtotal) - parseFloat(prevtotalprice) + parseFloat(currrowtotal); //console.log(currgrandtotal);
  //update totals 
  jpost(this).parent().parent().find('.totalprice').text(currrowtotal.toFixed(2));
  jpost('.sels__stab__comnt_right .sub-total').text(currsubtotal.toFixed(2));
  jpost('.sels__stab__comnt_right .sales-tax').text(currtaxtotal.toFixed(2));
  jpost('.sels__stab__comnt_right .grand-total').text(currgrandtotal.toFixed(2));
  //apply discount is applicable
  var pcdiscount = parseInt(jpost('#promocodeval').val());
  if(pcdiscount != ''){ if(pcdiscount > 0){ jpost('#applycc').trigger('click'); }}  

  //updating the session values
  jpost.post("sale_ui.php", { updtses:true,updaterow:pid,rowtotal:currrowtotal,quantity:enteredqty})
	.done(function(data){ console.log(data); });
  jpost.post("sale_ui.php", { updtses:true,grandtotal:currgrandtotal,subtotal:currsubtotal,taxtotal:currtaxtotal})
	.done(function(data){ console.log(data); }); 
});
...
jpost('#applycc').click(function(){
  var curr_subtotalretail = 0.00;
  jpost('.search__table tbody tr').each(function(){
    if(jpost(this).find('td.uprice').text() != 0.00){
      curr_subtotalretail += parseFloat(jpost(this).find('td.uprice').text());
    } else{
      curr_subtotalretail += parseFloat(jpost(this).find('td.totalprice').text());	
    } 
  });
...
<div class="modal fade" id="addproductforsale" tabindex="-1" role="dialog" aria-labelledby="addproductforsaleLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
	...
	<?php //creates a form object
     ...
     <script>
      $jqObj = jQuery.noConflict(); $i = 0;
      $jqObj('#addproductforsale').on('shown.bs.modal', function(){ 
	$jqObj('#addproductforsale .modal-body input[type="text"]').each(function(){ 
           if($jqObj(this).attr('name') == 'product_name'){ $jqObj(this).css('width', '100%'); }
           if($jqObj(this).attr('name') == 'product_quantity'){ $jqObj(this).css('width', '100%'); }
           if($jqObj(this).attr('name') == 'product_gross_total'){ $jqObj(this).css('width', '100%'); }
	}); 
	$jqObj('#addproductforsale .modal-body textarea').each(function(){ 
           if($jqObj(this).attr('name') == 'product_comments'){ $jqObj(this).css('width', '100%'); }
	...
	$jqObj('#addproductforsale .modal-body textarea').each(function(){ 
  	...
  	var licarthtml = createCartRowModal(pname, pquantity, pgrosstotal, pcomments);
       	if($jqObj(licarthtml).closest('tr').attr('id') > 0){ 
	   $jqObj('.searchtable center').css('display', 'none'); 
	}
  	jpost('.search__table tbody').append(licarthtml); 

        //make calculations and add to grand total 
	var uprice = $jqObj(licarthtml).find('td.uprice').text(); //console.log(uprice); 
	var taxpc = $jqObj(licarthtml).find('td.taxpc').text(); //console.log(taxpc);
	var quant = $jqObj(licarthtml).find('input[type="number"]').val(); //console.log(quant);
	var price = $jqObj(licarthtml).find('td.totalprice').text(); //console.log(price);
	//cals tax
	...
	$jqObj('#addproductforsale').modal('hide');
      ...
      function createCartRowModal(pname, pquantity, pgrosstotal, pcomments){
  	var $html=''; 
  	var uprice = parseFloat(0.00); var retailtaxpc = parseInt(0);
  	var retailprice = parseFloat(pgrosstotal); var pid = 1000001+($i++);
  	//append values
  	$html = '<tr id="'+pid+'">';
  	$html += '<td class="pronamedtls"><span class="d-block">'+pname+'/'+pcomments+'</span></td>';
  	$html += '<td class="uprice">'+uprice.toFixed(2)+'</td><td class="taxpc">'+retailtaxpc+'</td>';
  	...
  	$html += '</tr>'; //console.log($html); 
  	//updating the session values
  	sale_items = ';;'+pid+','+pname+'/'+pcomments+','+uprice+','+retailtaxpc+','+'1'+','+retailprice+';;';	
  	$jqObj.post("sale_ui.php", { updtses : true, updtsession : sale_items })
       		.done(function(data){ console.log(data); });
  	return $html;
      }
     </script>
