...
*/
  ...
  function tep_get_customers($customers_array = '') {
    ...
    $customers_query = tep_db_query("SELECT customers_id, customers_firstname, customers_lastname, customers_notes FROM " . TABLE_CUSTOMERS_RETAIL . 
									" WHERE customers_notes != '' ORDER BY customers_id DESC ");
...
<?php   
  ...
  print ("<tr class=\"dataTableHeadingRow\">
	...
	while ( $row = tep_db_fetch_array($customers_wishlist) ) { 
		//get wishlist product ids
		$wproductids = $row['customers_notes']; $wishlist_pids = explode(',' , $wproductids);
		$wishlists = array_filter($wishlist_pids); 
		foreach($wishlists as $wpId){ 
			if(! is_numeric($wpId)){ continue; }
...
$jqry('#send_email').click(function(){ 
 //alert('this');
 var records = []; 
